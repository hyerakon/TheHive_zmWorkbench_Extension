class TH_CS_CB_Bench_Modular_Pack : RecipeBase
{
    override void Init()
	{
		m_Name = "#STR_TH_CS_CB_MODULAR_DISMOUNT";
		...
		InsertIngredient(0, "TH_CS_CB_Bench_Modular");
		...
		//result - the kit
		AddResult("TH_CS_CB_Kit_Modular");
    ...
    override bool CanDo(ItemBase ingredients[], PlayerBase player)
	{
		...
		TH_CS_CB_Bench_Modular workbench = TH_CS_CB_Bench_Modular.Cast(ingredients[0]);
		if (!workbench)
			return false;
		if (workbench.IsKindOf("TH_CS_CB_Bench_Locked"))
		{
			return false;
		}
