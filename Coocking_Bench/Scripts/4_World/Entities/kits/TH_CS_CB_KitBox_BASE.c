class TH_CS_CB_KitBox_BASE : DeployableContainer_Base 
{
    protected Object TH_CS_CB_KitBox_ProtectedVar;
    
    protected string GetDeployObjectType()
    {
        return "TH_CS_CB_Bench_BASE";
    };
    ...
    override void OnPlacementComplete(Man player, vector position, vector orientation)
    {
		...
		if (!deployedItem)
		{
			Error("TH_CS_CB_KitBox_BASE - failed to deploy classname: " + deployedItemType);
			return;
		}
