class TheHive_CS_LB_Bench_Modular_Pack : RecipeBase
{
    override void Init()
	{
		m_Name = "#STR_THEHIVE_CS_LB_MODULAR_DISMOUNT";
		...
		InsertIngredient(0, "TheHive_CS_LB_Bench_Modular");
		...
		//result - the kit
		AddResult("TheHive_CS_LB_Kit_Modular");
    ...
    override bool CanDo(ItemBase ingredients[], PlayerBase player)
	{
		...
		TheHive_CS_LB_Bench_Modular workbench = TheHive_CS_LB_Bench_Modular.Cast(ingredients[0]);
		if (!workbench)
			return false;
		if (workbench.IsKindOf("TheHive_CS_LB_Bench_Locked"))
		{
			return false;
		}
	...
	{
		Debug.Log("#STR_THEHIVE_CS_LB_DEBUG_DISMOUNT_MSG", "[THEHIVE - CRAFTING SYSTEM]: Modular Tecnicbench Dismount");
	}
